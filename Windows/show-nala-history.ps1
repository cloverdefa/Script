...
function Update-VM {
  param (
    ...
    <# 顯示正在更新的伺服器名稱 #>
    Write-Host "執行 $hostname 伺服器指令" -ForegroundColor Yellow

    <# 使用 SSH 命令執行伺服器更新作業 #>
    ssh $hostname 'sudo nala history'

    <# 檢查是否有更新錯誤，如果有，顯示錯誤訊息 #>
    if ($LASTEXITCODE -ne 0) {
      Write-Host "執行 $hostname 伺服器指令時出現錯誤" -ForegroundColor Red
    }
    else {
      Write-Host "執行 $hostname 伺服器指令完成" -ForegroundColor Green
    }
...
if ($success) {
  Write-Output ("-" * 50)
    Write-Host "==== 執行全部伺服器指令完成 ====" -ForegroundColor Green
} else {
  Write-Output ("-" * 50)
    Write-Host "==== 執行伺服器指令出現錯誤 ====" -ForegroundColor Red
}
