...
# 定義要檢查的儲存庫列表
repos=("${(@f)$(<$HOME/Documents/github/Script/repositories.txt)}")

# 函數來執行 Git 操作
function Git-Pull-Repo() {
    local repo_name="$1"
    local repo_path="$HOME/Documents/github/$repo_name" # 使用 $HOME 環境變數
    local text="$repo_name 拉取遠端資料"

    cd "$repo_path" || exit 1 # 切換到儲存庫目錄如果切換失敗，退出腳本，並返回退出碼1
